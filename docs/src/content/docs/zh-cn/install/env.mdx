...
### `GUKHANMUN` <Badge text="可选" />

以逗号分隔的语言区域标签模式列表，用于指定哪些本地帖子应通过
[Gukhanmun]为汉字添加韩文读音。匹配不区分大小写，`*`可匹配任意长度的
字符串。例如，`GUKHANMUN=ko,ko-*`可匹配`ko`、`ko-KR`、
`ko-Kore-KR`和`ko-KP`。

`ko-*`模式不匹配单独的`ko`标签，因此若要处理这两种形式，需要同时列出
两个模式。区域子标签为`KP`的语言区域使用朝鲜式读音，其他匹配的语言区域
使用韩国式读音。

如果未设置此变量或将其留空，Hollo不会加载Gukhanmun。它只处理新建或编辑的
本地帖子，不会更改现有帖子或远程帖子。加载或转换失败时，帖子请求也会失败。
...
### `REMOTE_EMOJI_REACTIONS_FETCH_MAX_ITEMS` <Badge text="可选" />

保存远程帖子时，从远程 `emojiReactions` 集合导入并保存的表情回应项目
最大数量。设置为 `0` 可禁用远程表情回应导入。
