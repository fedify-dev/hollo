...
要部署 Hollo，您需要 S3 或兼容 S3 的对象存储来存储媒体文件，如图片。目前有许多兼容 S3 的对象存储服务，包括 AWS S3、Cloudflare R2、RustFS、DigitalOcean Spaces 和 Linode Object Storage。准备好对象存储后，您需要适当地配置环境变量（请参阅各服务的 S3 客户端 API 使用方法）。更多信息请参见[**环境变量**](/zh-cn/install/env)章节。
