...
### `GUKHANMUN` <Badge text="オプション" />

[Gukhanmun]で漢字にハングルの読みを付けるローカル投稿のロケールタグ
パターンを、カンマ区切りで指定します。パターンは大文字と小文字を区別せず、
`*`は任意の長さの文字列に一致します。たとえば`GUKHANMUN=ko,ko-*`は、
`ko`、`ko-KR`、`ko-Kore-KR`、`ko-KP`に一致します。

`ko-*`パターンは`ko`自体には一致しないため、両方を処理する場合は両方の
パターンを指定してください。地域サブタグが`KP`のロケールでは北朝鮮式の
読みを使用し、それ以外の一致するロケールでは韓国式の読みを使用します。

この変数が未設定または空の場合、Gukhanmunは読み込まれません。新規作成または
編集されたローカル投稿だけを処理し、既存の投稿とリモート投稿は変更しません。
Gukhanmunの読み込みまたは変換に失敗すると、投稿リクエストも失敗します。
...
### `REMOTE_EMOJI_REACTIONS_FETCH_MAX_ITEMS` <Badge text="オプション" />

リモート投稿を保存するときに、リモートの`emojiReactions`コレクション
から読み込んで保存する絵文字リアクションアイテムの最大数。
`0`に設定すると、リモート絵文字リアクションの取得を無効にします。
