...
### `GUKHANMUN` <Badge text="Optional" />

A comma-separated list of locale tag patterns for local posts that should have
Hangul readings added to Hanja by [Gukhanmun].  Matching is case-insensitive,
and `*` can stand for any sequence of characters.  For example,
`GUKHANMUN=ko,ko-*` matches `ko`, `ko-KR`, `ko-Kore-KR`, and `ko-KP`.

The `ko-*` pattern does not match the bare `ko` tag, so list both when both
forms should be processed.  Locales with a `KP` region subtag use North Korean
readings; other matching locales use South Korean readings.

If this variable is unset or empty, Gukhanmun is not loaded.  It processes only
local posts when they are created or edited.  Existing and remote posts are not
changed.  A loading or conversion error causes the post request to fail.
...
### `REMOTE_EMOJI_REACTIONS_FETCH_MAX_ITEMS` <Badge text="Optional" />

The maximum number of emoji reaction items to import from a remote
`emojiReactions` collection when a remote post is persisted.
Set this to `0` to disable remote emoji reaction imports.
