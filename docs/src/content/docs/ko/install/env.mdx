...
### `GUKHANMUN` <Badge text="선택" />

[Gukhanmun]으로 한자에 한글 독음을 붙일 로컬 게시물의 로캘 태그 패턴을
쉼표로 구분한 목록입니다. 패턴은 대소문자를 구분하지 않으며, `*`는 길이에
상관없이 모든 문자열에 일치합니다. 예를 들어 `GUKHANMUN=ko,ko-*`는
`ko`, `ko-KR`, `ko-Kore-KR`, `ko-KP`에 일치합니다.

`ko-*` 패턴은 `ko` 자체에는 일치하지 않으므로 둘 다 처리하려면 두 패턴을
모두 나열해야 합니다. 지역 하위 태그가 `KP`인 로캘에는 북한식 독음을,
그 밖의 일치하는 로캘에는 남한식 독음을 사용합니다.

이 변수를 설정하지 않거나 빈 값으로 두면 Gukhanmun을 불러오지 않습니다.
새로 작성하거나 수정하는 로컬 게시물만 처리하며, 기존 게시물과 원격 게시물은
바꾸지 않습니다. Gukhanmun을 불러오거나 변환하는 데 실패하면 게시물 요청도
실패합니다.
...
### `REMOTE_EMOJI_REACTIONS_FETCH_MAX_ITEMS` <Badge text="선택" />

원격 게시물을 저장할 때 원격 `emojiReactions` 컬렉션에서 가져와 저장할
이모지 반응 항목의 최대 수입니다. `0`으로 설정하면 원격 이모지 반응
가져오기를 비활성화합니다.
